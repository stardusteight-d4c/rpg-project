        ...
        {[].map((item, index) => (
          <div key={index} className="col-span-1">
            <img
              src={item}
              alt=""
              className="rounded-lg w-full h-[99px] cursor-pointer overflow-hidden aspect-square"
            />
          </div>
        ))}
        <div
          onClick={() => setOpenModal("open")}
          className="col-span-1 cursor-pointer hover:brightness-150 flex items-center justify-center h-[99px] bg-border/80 rounded-lg"
        >
          <svg
