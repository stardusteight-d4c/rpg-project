...
export const ProfileInfoEdit: React.FC<{ character: ISheet }> = ({
  ...
  function handleType() {
    if (currentSession.role === "player") return "player"

    if (currentSession.role === "master") {
      if (editableData.type === "npc") return "enemy"
      if (editableData.type === "enemy") return "npc"
      if (editableData.type === "player") return "player"
    }
  }
          ...
          <div className="flex absolute top-2 z-40 left-2 items-center w-fit gap-x-2">
            ...
            >
              <button
                ...
                className="bg-background flex !cursor-default items-center justify-center text-white p-1 rounded-full shadow-md shadow-black/50"
              >
